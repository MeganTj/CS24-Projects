...
void sthread_start(int timer) {
    if (timer)
        start_timer();
    /* 
     * Lock to ensure that the timer does not interrupt while the current 
     * thread is using the scheduler to start its operations.
     */
    __sthread_lock();
    __sthread_start();
}
...
void __sthread_finish(void) {
    /*
     * Lock before switching the context to prevent the timer from switching
     * control while the current thread is using the scheduler to do so.
     */
    __sthread_lock();
...
void sthread_block() {
    /*
     * Lock before switching the context to prevent the timer from switching
     * control while the scheduler is used to block the current thread.
     */
    __sthread_lock();
...
void sthread_unblock(Thread *threadp) {
    ...
    /*
     * All the steps to unblock a thread has been completed, so
     * the thread is done using the scheduler.
     */
    __sthread_unlock();
}
