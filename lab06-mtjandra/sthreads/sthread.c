...
ThreadContext *__sthread_scheduler(ThreadContext *context) {
    ...
    if (context != NULL) {
         ...
         if (state == ThreadRunning) {
             /* 
              * If there is one or fewer threads in the system, just 
              * resume running.
              */
         ...
         else if (state == ThreadFinished){
             __sthread_delete(current);
         }
    }
   
    /* Selects the next thread from the ready queue.*/
    if (queue_empty(&ready_queue) == 0) {
        ...
        if (queue_empty(&blocked_queue) == 1) {
            fprintf(stderr, "Success! All threads completed.\n");
            exit(0);
        }
        fprintf(stderr, "Not all threads completed successfully.\n");
        exit(1);
    }
    
    /* Returns the next thread to resume executing. */
    return current->context;
}
...
Thread * sthread_create(void (*f)(void *arg), void *arg) {
    char *stack = (char *) malloc(DEFAULT_STACKSIZE);
    if (stack == NULL) {
    ...
    /* stack now points to the end of the allocated stack. */
    stack += DEFAULT_STACKSIZE;
    new_thread->context = __sthread_initialize_context((void *) stack, f, arg);
